...
function updateGoalUI() {
  ...
  // Visual fill is clamped to 100% so the dog and bar stay anchored at the bone
  const visualFillPercentage = Math.min(percentageRounded, 100);
  const fill = document.getElementById('progress-bar-fill');
  const dog = document.getElementById('doggo-runner');
  const bone = document.getElementById('target-bone');
  
  if (fill) fill.style.width = `${visualFillPercentage}%`;
  if (dog) dog.style.left = `calc(${visualFillPercentage}% - 16px)`;
